{{ config(materialized='view', alias='_v_gt_employees') }}
with 
  current_employment_history as (
    select * from {{ source('postgres_public', 'employment_histories') }}
    where id in (
      select
...
--   , c.status
  ...
  , c.peo_member_id
from 
    {{source('postgres_public', 'peo_connections')}} c
    left join {{source('postgres_public', 'members')}} h 
    ...
    left join {{source('postgres_public', 'organisations')}} ho on 
        h.organisation_id = ho.id
        and not ho._fivetran_deleted
        and not ho.is_shadow_data
--   left join postgres_public.employing_entities ee on 
--     h.organisation_id = ee.organisation_id
--     and not ee._fivetran_deleted
    left join {{source('postgres_public', 'members')}}  p on 
        c.peo_member_id = p.id
        and not p._fivetran_deleted
        and not p.is_shadow_data
    left join  {{source('postgres_public', 'organisations')}} po on 
        p.organisation_id = po.id
        and not po._fivetran_deleted
        and not po.is_shadow_data
where
  not c._fivetran_deleted
  and ({{legit_emails('u.email')}} or {{legit_emails('h.personal_email')}} )
