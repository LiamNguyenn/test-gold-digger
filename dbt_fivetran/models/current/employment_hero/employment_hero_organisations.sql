  ...
  select
    ...
    from
      (
        ...
        from
          postgres_public.members as m
        join postgres_public.users as u on
          m.user_id = u.id
        where
...
, churn_date_field as (
    ...
    from
      {{ source('zuora', 'subscription') }} s
      join {{ source('zuora', 'account') }} a on
        s.account_id = a.id
    where
...
--   addresses.postcode,
  ...
  , case when sc.id is not null and za.batch is not null and za.batch != 'Batch50' and (sc.is_churn > 0 or getdate() < cdf.churn_date or cdf.churn_date is null) and s.pricing_tier not ilike '%free%' then true    
    --when o.id in (select host_organisation_id from {{ref('employment_hero_v_gt_active_organisations')}}) then true 
...
from 
  ...
  left join {{ref('employment_hero_v_last_connected_payroll')}} as p
    on o.id = p.organisation_id
  left join {{ source('postgres_public', 'industry_categories') }} as i
    on o.industry_category_id = i.id
      and not i._fivetran_deleted
  left join {{ source('postgres_public', 'salesforce_accounts') }} as sa
    on o.id = sa.organisation_id
      and not sa._fivetran_deleted
  left join {{ source('zuora', 'account') }} za on o.zuora_account_id = za.id and not za._fivetran_deleted   
...
--   left join postgres_public.addresses
--     on o.primary_address_id = addresses.id
where 
