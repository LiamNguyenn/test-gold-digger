...
from
    {{ref('keypay_business_traits')}} b    
    left join {{ ref('keypay_industry') }} i on b.industry_id = i.id
    left join {{ ref('keypay_zoom_info') }} z on b.id = z._id
    left join (
            select business_id, max(billing_month::date) as last_billing_month from {{ ref('keypay_t_pay_run_total_monthly_summary') }} group by 1
