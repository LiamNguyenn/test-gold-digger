...
, business_organisation_overlap as (
    ...
    from
      (
        select
          ...
          , external_id
        from
          {{ref('employment_hero_v_last_connected_payroll')}}
          as epa
          left join {{ source('postgres_public', 'payroll_infos') }} on
      ...
      as o
      join {{ ref('keypay_dwh_business') }} b on
        b.id = o.external_id
  )
  , keypay_employees as (
    select distinct
    ...
    from 
      {{ ref('keypay_au_pay_run_summary_s') }} e
      join 
        (select 
          employee_id
          , min(billing_month) as first_paid_month
        from
          {{ ref('keypay_au_pay_run_summary_s') }}
        group by 1) b 
  ...
  , business_growth_rate as (
    ...
    from
      ...
      join(
        ...
        JOIN {{ ref('keypay_white_label') }} AS wl ON b.white_label_id = wl.id       
          left join {{ ref('keypay_dwh_suburb') }} s on
            b.suburb_id = s.id
          left join {{ ref('keypay_industry') }} i on
            b.industry_id = i.id
      ) b on
        m.business_id = b.id
      left join {{ ref('keypay_zoom_info') }} z on
        m.business_id = z._id
  ...
  from months m 
    join {{ ref('employment_hero_employees') }} e on m.date >= coalesce(e.start_date, e.created_at)
      and (m.date <= e.termination_date or e.termination_date is null)
  ...
  , organisation_growth_rate as (
    ...
    from
      ...
      left join {{ ref('employment_hero_organisations') }} as o on
        o.id = m.organisation_id
      left join {{ source('postgres_public', 'addresses') }} a on
