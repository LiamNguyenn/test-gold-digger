{{ config(materialized="view", alias="v_median_hours_worked_kp") }}
...
with
    kp_business_industry as (
        ...
        from
            (
                ...
                from {{ ref('keypay_dwh_business') }} as b
                left join
                    (select id, name from {{ ref('keypay_industry') }}) as i on b.industry_id = i.id
                left join
                    (select _id, primary_industry from keypay.zoom_info) as z
                    on b.id = z._id
            ) as m
        left join
            one_platform.industry as i
    ...
    keypay_employees as (
        ...
        join {{ ref('keypay_dwh_business') }} as b on b.id = e.business_id
        left join {{ ref('keypay_white_label') }} as wl on b.white_label_id = wl.id
        left join
            {{ ref('keypay_tax_file_declaration') }} as tfd
            on tfd.id = e.tax_file_declaration_id
            and e.id = tfd.employee_id  -- AU
        left join {{ ref('keypay_employment_type') }} as et on et.id = tfd.employment_type_id
        where
            (
                e.end_date is null
    ...
    employee_details as (
        ...
        left join {{ ref('keypay_dwh_suburb') }} s on m.residential_suburb_id = s.id
    ...
    keypay_pay_run_summary as (
        ...
        left join
            {{ ref('keypay_payrun_total_history') }} prth
...
--                 or b.to_be_deleted = 'False'
    ...
    billed_business as (
        ...
        from {{ ref('keypay_invoice_line_item') }} ili
        join
            {{ ref('keypay_invoice') }} i
            on i.id = ili.invoice_id
            and split_part(i._file, 'Shard', 2) = split_part(ili._file, 'Shard', 2)
        left join
            {{ ref('keypay_dwh_business') }} b
            on ili.abn = b.abn
...
join {{ ref('keypay_dwh_business') }} b on mh.business_id = b.id
left join {{ ref('keypay_white_label') }} as wl on b.white_label_id = wl.id
join
