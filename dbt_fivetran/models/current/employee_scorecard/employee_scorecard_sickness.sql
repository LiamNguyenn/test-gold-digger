{{ config(materialized="table", alias="sickness_scorecard") }}
...
with
    cohort_w_attributes as (
        ...
        from 
            {{ref('employee_scorecard_cohort')}} as e
            left join {{ref('employee_scorecard_bradford_factor')}} as bf on
                e.member_id = bf.member_id
            left join {{ref('employee_scorecard_absenteeism')}} as ab on
