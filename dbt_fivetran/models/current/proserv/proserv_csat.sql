...
select distinct
    ...
    pj.imp_proj_id
from
    {{ source('delighted_proserv_csat', 'response') }} as r
inner join {{ source('delighted_proserv_csat', 'person') }} as s
    on
        r.person_id = s.id
left join (
    select
    ...
    from
        {{ source('salesforce', 'asana_public_asana_projects_relation_c') }} as ap
    inner join {{ source('salesforce', 'implementation_project_c') }} as imps
        on
