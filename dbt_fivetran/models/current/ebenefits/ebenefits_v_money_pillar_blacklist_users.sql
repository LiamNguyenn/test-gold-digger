{{ config(materialized='view', alias='_v_money_pillar_blacklist_users') }}
...
with blacklisted_org_users as (
    ...
    left join {{ref('ebenefits_v_money_pillar_blacklist_organisations')}} bo on bo.organisation_id = m.organisation_id
...
from (
    ...
    join {{ source('postgres_public', 'users') }} u on d.user_email = u.email 
