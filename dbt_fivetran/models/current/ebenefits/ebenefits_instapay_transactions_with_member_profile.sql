{{ config(materialized='incremental', alias='instapay_transactions_with_member_profile') }}
...
with timesheets as (
  ...
  , sum(units) as units
  from {{ source('heropay_db_public', 'timesheets') }} 
...
, payslips as (
  ...
  from  {{ source('postgres_public', 'payslips') }} ps
...
, salaries as (
  ...
  from {{ source('postgres_public', 'salary_versions') }} 
  where not _fivetran_deleted
  and salary > 0    
...
join {{ ref('employment_hero_employees')}} as m on m.uuid = ht.member_id
join {{ ref('employment_hero_v_active_employees_by_organisations')}} os on m.organisation_id = os.organisation_id  
join {{ ref('employment_hero_organisations')}} o on m.organisation_id = o.id 
left join {{ source('heropay_db_public', 'heropay_balances') }} b on b.id = ht.heropay_balance_id and not b._fivetran_deleted
left join {{ source('heropay_db_public', 'member_infos') }} mi on mi.heropay_balance_id = b.id and not mi._fivetran_deleted
left join {{ source('one_platform', 'industry') }} as i on regexp_replace( o.industry,'\\s','') = regexp_replace( i.eh_industry,'\\s','')
...
{% if is_incremental() %}
        and ht.created_at > (SELECT MAX(transaction_date) FROM {{ this }} ) 
{% endif %} 
