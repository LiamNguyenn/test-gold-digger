{{ config(materialized='view', alias='_v_swag_store_on_off_users') }}
...
with eh_org_users as (
    ...
    join {{ source('postgres_public', 'organisations') }} o on m.organisation_id = o.id 
    left join {{ref('ebenefits_v_worklife_refused_organisations')}} bo on bo.organisation_uuid = m.organisation_id
