{{ config(alias='instapay_eligible_member_profile') }}
...
with 
    payslips as (
        ...
        from 
            {{ source('postgres_public', 'payslips') }} ps
        where 
    ...
    , salary_version AS (
        ...
        from 
            {{current_row('postgres_public', 'salary_versions', 'member_id')}} as sv
            left join {{ source('postgres_public', 'pay_schedules') }} as ps on
    ...
    , unlaunched_organisations AS (
        select
            organisation_id
        from
            {{ source('postgres_public', 'sandbox_settings') }}
        where
    ...
    , heropay_transactions_by_member AS (
        ...
        from
            {{ source('heropay_db_public', 'heropay_transactions') }}
        where 
    ...
    , instapay_members as (
        ...
        from 
            {{ ref('employment_hero_employees')}} as m
            join {{ source('postgres_public', 'payroll_infos') }} as i on 
            ...
            left join 
                ...
                mp.member_id = m.id
            left join {{source('postgres_public', 'addresses')}} ea on 
                m.address_id = ea.id and not ea._fivetran_deleted 
            left join heropay_transactions_by_member hp on
                hp.member_uuid = m.uuid
            left join {{ref('employment_hero_v_employees_first_time_swag_app')}} as sa on
            ...
            join {{ref('ebenefits_v_instapay_on_off_organisations')}} ioo on 
                ioo.organisation_id = m.organisation_id
                and ioo.instapay_enabled
            join {{ source('postgres_public', 'menu_customisations') }} as mc on 
                mc.organisation_id = o.id
                and mc.instapay != 0
            left join  {{ ref('employment_hero_v_active_employees_by_organisations')}} os on 
                m.organisation_id = os.organisation_id  
            left join  {{ source('one_platform', 'industry') }} as oi on 
