...
with
    eh_hr_accounts as (
        ...
        inner join {{ source("salesforce", "account") }} sa on za.crm_id = sa.id
        inner join {{ source("zuora", "subscription") }} zs on zs.account_id = za.id
        left join {{ source("zuora", "rate_plan_charge") }} zrpc on zs.id = zrpc.subscription_id
        left join {{ source("zuora", "product_rate_plan") }} zprp on zrpc.product_rate_plan_id = zprp.id
        left join {{ source("zuora", "product") }} zp on zprp.product_id = zp.id
        where
    ...
    stage_churn as (
        ...
        left join {{ source("zuora", "product") }} zp on zprp.product_id = zp.id
        left join
            {{ source("postgres_public", "organisations") }} o
            on za.id = o.zuora_account_id
            and not o._fivetran_deleted
            and not o.is_shadow_data
        left join
            {{ source("postgres_public", "business_accounts") }} ba
    ...
    churn_date_field as (
        ...
        inner join {{ source("salesforce", "account") }} sa on sa.id = scn.external_id
        inner join {{ source("zuora", "account") }} za on za.crm_id = sa.id
        inner join {{ source("zuora", "subscription") }} zs on zs.account_id = za.id
        join {{ source("zuora", "rate_plan_charge") }} zrpc on zs.id = zrpc.subscription_id
        join {{ source("zuora", "product_rate_plan") }} zprp on zrpc.product_rate_plan_id = zprp.id
        join {{ source("zuora", "product") }} zp on zprp.product_id = zp.id
        inner join
            (
                ...
                join {{ source("zuora", "account") }} a on s.account_id = a.id
    ...
    stage_implementation as (
        ...
        from stage_churn as scn
        left join
            {{ source("salesforce", "implementation_project_c") }} ipc_hr
        ...
        left join
            {{ source("salesforce", "implementation_project_c") }} ipc_pr
    ...
    stage_completed as (
        ...
        inner join {{ source("zuora", "account") }} za on za.crm_id = si.external_id
        left join paying_eh_orgs o on za.id = o.zuora_account_id
        left join {{ source("salesforce", "keypay_org_c") }} kp on si.external_id = kp.linked_account_c
    ...
    account_stages as (
        ...
        left join {{ source("salesforce", "account") }} sa on scn.external_id = sa.id and not sa.is_deleted
    ...
    subscription as (
        ...
        join {{ source("zuora", "account") }} a on a.crm_id = eh.external_id
        join {{ source("zuora", "subscription") }} s on s.account_id = a.id
        join {{ source("zuora", "rate_plan_charge") }} rpc on rpc.subscription_id = s.id
    ...
    account_contract_details as (
        ...
        from
            (
                ...
                from {{ source("salesforce", "account") }} sa
                join {{ source("zuora", "account") }} za on za.crm_id = sa.id
                join {{ source("zuora", "subscription") }} zs on zs.account_id = za.id
                join {{ source("zuora", "rate_plan_charge") }} zrpc on zrpc.subscription_id = zs.id
                join {{ source("zuora", "product_rate_plan") }} zprp on zrpc.product_rate_plan_id = zprp.id
                join {{ source("zuora", "product") }} zp on zp.id = zprp.product_id
                where
                    not za._fivetran_deleted
                    and not zs._fivetran_deleted
                    and not zrpc._fivetran_deleted
    ...
    account_finance as (
        ...
        join {{ source("zuora", "account") }} a on st.external_id = a.crm_id
        join {{ source("zuora", "rate_plan_charge") }} rpc on a.id = rpc.account_id
        join
            (
        ...
        join {{ source("zuora", "product_rate_plan") }} prp on rpc.product_rate_plan_id = prp.id
        join {{ source("zuora", "product_rate_plan_charge") }} prpc on prp.id = prpc.product_rate_plan_id
        join {{ source("zuora", "product") }} p on prp.product_id = p.id
        join
            {{ source("zuora", "product_rate_plan_charge_tier") }} prpct
    ...
    account_mrr as (
        ...
        from
            (
                ...
                join {{ source("zuora", "invoice") }} on account.id = invoice.account_id
                join {{ source("zuora", "invoice_item") }} on invoice.id = invoice_item.invoice_id
                join {{ source("zuora", "product") }} zp on invoice_item.product_id = zp.id
                where
    ...
    account_employees as (
        ...
        join {{ source("zuora", "account") }} za on za.crm_id = sa.id
        left join paying_eh_orgs o on za.id = o.zuora_account_id
        left join {{ source("postgres_public", "members") }} m on o.id = m.organisation_id
        inner join {{ source("postgres_public", "users") }} u on u.id = m.user_id
        where
            not sa.is_deleted
            and not za._fivetran_deleted
            and not o._fivetran_deleted
    ...
    account_opp_item as (
        ...
        left join {{ source("salesforce", "opportunity_line_item") }} oppl on opp.id = oppl.opportunity_id
        left join {{ source("salesforce", "user") }} su on opp.owner_id = su.id
    ...
    implementation_info as (
        ...
        from {{ source("salesforce", "implementation_project_c") }} ipc
        join {{ source("salesforce", "account") }} sa on sa.id = ipc.account_c
    ...
    account_imps_hr_projects as (
        ...
        from {{ source("salesforce", "implementation_project_c") }} ipc
        join {{ source("salesforce", "eh_org_c") }} eh on ipc.id = eh.professional_service_project_c
        where
            not ipc.is_deleted
    ...
    account_imps_payroll_projects as (
        ...
        from {{ source("salesforce", "implementation_project_c") }} ipc
        join {{ source("salesforce", "keypay_org_c") }} py on ipc.id = py.professional_service_project_c
        left join
            {{ source("postgres_public", "payroll_infos") }} pi on py.id_c = pi.external_id and not pi._fivetran_deleted
        left join {{ ref("employment_hero_v_connected_payrolls") }} epa on pi.id = epa.payroll_info_id
        where
            not ipc.is_deleted
    ...
    account_support_tickets as (
        ...
        join {{ source("zendesk", "group") }} g on t.group_id = g.id
        join {{ source("zendesk", "user") }} u on t.assignee_id = u.id
        join {{ source("zendesk", "organization") }} o on t.organization_id = o.id
        join
            {{ source("postgres_public", "organisations") }} po
            on o.custom_organisation_id = po.id
            and not po._fivetran_deleted
            and not po.is_shadow_data
        join {{ source("zuora", "account") }} za on po.zuora_account_id = za.id
    ...
    account_feature_requests as (
        select
            ...
            || ']' as feature_requests
        from account_support_tickets t
        join {{ source("zendesk", "ticket_link") }} tl on t.id = tl.ticket_id
        join {{ source("jira", "issue") }} i on tl.issue_id = i.id
    ...
    account_primary_contact as (
        ...
        join {{ source("zuora", "account") }} za on c.account_id = za.crm_id
        join {{ source("postgres_public", "organisations") }} o on za.id = o.zuora_account_id
        join {{ source("postgres_public", "members") }} m on o.id = m.organisation_id
            ...
            (
                with
                    product_variants2 as (
                        ...
                        from {{ source("heroshop_db_public", "product_variants") }}
                    )
                ...
                select
                    ...
                    od.status
                from {{ source("heroshop_db_public", "order_details") }} od
                ...
                join
                    ...
                    and not m.is_shadow_data
                join
                    {{ source("postgres_public", "organisations") }} org
            ...
            (
                with
                    imps_incentive as (
                        ...
                        from {{ source("herodollar_service_public", "hero_dollar_transactions") }}
                        join
                            {{ source("herodollar_service_public", "tracking_infos") }}
                ...
                from
                    (
                        ...
                        from {{ source("herodollar_service_public", "hero_dollar_transactions") }} t
                        join
                            {{ source("postgres_public", "organisations") }} o
    ...
    account_imps_payroll_days as (
        ...
        from {{ source("asana", "task") }} t
        join {{ source("asana", "project_task") }} pt on t.id = pt.task_id
        join
            {{ source("salesforce", "asana_public_asana_projects_relation_c") }} ap
            on pt.project_id = ap.asana_public_asana_project_id_c
        join {{ source("salesforce", "implementation_project_c") }} imp on ap.asana_public_object_id_c = imp.id
        join
            (
    ...
    accounts_zuora_payments as (
        ...
        from
            (
                ...
                join {{ source("zuora", "invoice") }} i on a.id = i.account_id
                join {{ source("zuora", "payment_application") }} pa on i.id = pa.invoice_id
                join {{ source("zuora", "payment") }} p on pa.payment_id = p.id
                where
                    not a._fivetran_deleted
    ...
    ipc_org as (
        ...
        from {{ source("salesforce", "implementation_project_c") }} ipc
        join {{ source("zuora", "account") }} za on za.crm_id = ipc.account_c
        left join
    ...
    account_payroll_platform as (
        ...
        join {{ source("postgres_public", "payroll_infos") }} pi on epa.payroll_info_id = pi.id
        where
            not za._fivetran_deleted
            and not o._fivetran_deleted
            and not o.is_shadow_data
    ...
    account_proserv_csat as (
        ...
        from
            (
                ...
                from {{ source("salesforce", "implementation_project_c") }}
                join
                    {{ source("salesforce", "asana_public_asana_projects_relation_c") }} ap
                    on ap.asana_public_object_id_c = implementation_project_c.id
                left join
                    {{ source("delighted_proserv_csat", "response") }} as kick_off_hr
                ...
                left join
                    {{ source("delighted_proserv_csat", "response") }} as kick_off_payroll
                ...
                left join
                    {{ source("delighted_proserv_csat", "response") }} as midway_hr
                ...
                left join
                    {{ source("delighted_proserv_csat", "response") }} as midway_payroll
                ...
                left join
                    {{ source("delighted_proserv_csat", "response") }} as completion_hr
                ...
                left join
                    {{ source("delighted_proserv_csat", "response") }} as completion_payroll
        ...
        from
            (
                select
                    ...
                    u.email,
                    u.uuid as user_uuid
                from {{ source("postgres_public", "members") }} as m
                join {{ source("postgres_public", "users") }} as u on u.id = m.user_id
                join {{ source("postgres_public", "organisations") }} as org on m.organisation_id = org.id
                join {{ source("zuora", "account") }} as za on org.zuora_account_id = za.id
                where
                    m.id in (
                        select
    ...
    account_sales_csat as (
        ...
        from {{ source("delighted_sales_csat", "response") }}
        join {{ source("salesforce", "opportunity") }} as opp on response.properties_opportunity_id = opp.id
        where
            not opp.is_deleted
            and response.id in (
                select
                ...
                from {{ source("delighted_sales_csat", "response") }} r
                join {{ source("salesforce", "opportunity") }} as opp on r.properties_opportunity_id = opp.id
            )
    )
...
left join
    {{ ref("mp_daumau_by_account") }} dma
    on dma.account_id = st.external_id
    and dma.date = (select max(date) from {{ ref("mp_daumau_by_account") }})
