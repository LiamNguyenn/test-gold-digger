...
user_info as (
    ...
    from {{ source('postgres_public', 'user_infos') }} as u
    left join {{ source('postgres_public', 'addresses') }} as addr
        on u.address_id = addr.id and not addr._fivetran_deleted
    where u.updated_at = (
        select max(updated_at) from {{ source('postgres_public', 'user_infos') }}
...
employee_latest as (
    select e1.*
    from {{ ref('employment_hero_employees') }} as e1
    where e1.updated_at = (
        select max(updated_at) from {{ ref('employment_hero_employees') }} as e2
...
employment_histories as (
    ...
    from {{ source('postgres_public', 'user_employment_histories') }}
...
users as (
    ...
    left join user_info as ui
        on u.id = ui.user_id
    left join {{ source('workshop_public', 'country_codes') }} as cc
...
user_braze_unsubscribed_lastest as (
    ...
    from (
        ...
        from {{ source('braze', 'subscription_event') }}
...
marketo_subscription as (
    ...
    from {{ source('marketo', 'subscription') }}
),
...
select
    ...
    {{ target.schema }}.format_phone_number_e164(u.phone_number, u.alpha_two_letter) as phone_number_e164,
