...
, employee_pay_category as (
  ...
  from -- latest default pay
    (
      select * from {{ ref('keypay_employee_pay_category') }}
        where id in
        (
              select
                FIRST_VALUE(id) over (
              ...
              from {{ ref('keypay_employee_pay_category') }} c
  ...
  join {{ ref('keypay_rate_unit') }} cu on c.pay_category_rate_unit_id = cu.id 
  join {{ ref('keypay_rate_unit') }} eu on c.employee_rate_unit_id = eu.id    
  join {{ ref('keypay_dwh_employee') }} e on c.employee_id = e.id 
  join recent_payrun pr on pr.employee_id = c.employee_id and billing_month_desc = 1  
   join keypay.payrun_default pd on e.pay_run_default_id = pd.id and pd.employee_id = e.id 
...
, business_eh_industry as (
    ...
    from 
      (
        ...
        from
          {{ ref('keypay_dwh_business') }} as b
          left join (select id, name from {{ ref('keypay_industry') }}) as i on
            b.industry_id = i.id
          left join (select _id, primary_industry from {{ ref('keypay_zoom_info') }}) as z on
            b.id = z._id
      ) as m
