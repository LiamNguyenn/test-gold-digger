...
with
  project_imp_table as (
    select distinct
      ...
      , ip.status_c
    from
      {{ source('salesforce', 'implementation_project_c') }} ip
      left join {{ source('salesforce', 'account') }} a on
        ip.account_c = a.id
    where
  ...
  , churn_table as (
    select
      ...
      , c.category_c
    from
      {{ source('salesforce', 'case') }} c
      left join {{ source('salesforce', 'account') }} ac on
        c.account_id = ac.id
    where
      ac.is_deleted = false
      and c.category_c in (
