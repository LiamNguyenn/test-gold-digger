{{ config(alias='_t_pay_run_total_monthly_summary') }}
...
from  {{ ref('int__keypay__payrun') }} pr
  	join {{ ref('int__keypay_dwh__business') }} b on pr.business_id = b.id AND SPLIT_PART(b._file, 'Shard', 2) = SPLIT_PART(pr._file, 'Shard', 2)
	left join {{ ref('int__keypay__payrun_total') }} prt on prt.payrun_id = pr.id and SPLIT_PART(pr._file, 'Shard', 2) = SPLIT_PART(prt._file, 'Shard', 2) and pr.date_first_finalised::date >= '2022-01-01' and prt.is_excluded_from_billing = 0
  	left join {{ ref('stg__keypay__payrun_total_history') }} prth on prth.payrun_id = pr.id and SPLIT_PART(pr._file, 'Shard', 2) = SPLIT_PART(prth._file, 'Shard', 2) and pr.date_first_finalised::date < '2022-01-01' and prth.is_excluded_from_billing = 0
  where
