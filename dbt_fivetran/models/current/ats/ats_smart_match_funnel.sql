{{ config(alias='smart_match_funnel') }}
...
from 
    ( select *, row_number() OVER (PARTITION BY org_uuid, user_uuid, case when raw_job_matched = '' then job_matched else raw_job_matched end ORDER BY "time" ASC) as rn
        from  {{ref('ats_smart_match_candidates_shown')}} 
    )s
...
from ( 
  ...
  join {{ref('employment_hero_organisations')}} o on s.org_uuid = o.uuid and o.is_paying_eh
  left join {{ref('employment_hero_employees')}} e on e.uuid = s.employer_member_uuid
    where s.rn = 1
)
...
from (
    select *, row_number() OVER (PARTITION BY org_uuid, user_uuid, case when raw_job_matched = '' then job_matched else raw_job_matched end ORDER BY "time" ASC) as rn
    from {{ref('ats_smart_match_candidates_shortlisted_actioned')}} 
    )s
  join {{ref('employment_hero_organisations')}} o on s.org_uuid = o.uuid and o.is_paying_eh
  left join {{ref('employment_hero_employees')}} e on e.uuid = s.employer_member_uuid
  where s.rn = 1
...
from (
    select *, row_number() OVER (PARTITION BY org_uuid, user_uuid, case when raw_job_matched = '' then job_matched else raw_job_matched end ORDER BY "time" ASC) as rn
    from {{ref('ats_smart_match_candidates_saved_actioned')}} 
    )s
  join {{ref('employment_hero_organisations')}} o on s.org_uuid = o.uuid and o.is_paying_eh
  left join {{ref('employment_hero_employees')}} e on e.uuid = s.employer_member_uuid
  where s.rn = 1
...
, base as (
    ...
    join {{ref('employment_hero_organisations')}} eho on x.org_id = eho.id
    left join {{source('postgres_public', 'addresses')}} a on eho.primary_address_id = a.id and not a._fivetran_deleted
    left join (
        select *
        from {{ source('postgres_public', 'address_geolocations') }}
        where id in (
...
, hired_from_saved_hired_at as (
    select org_uuid, org_id, user_uuid, candidate_email
