...
from
  ...
  join {{ ref('ats_jobs_created') }} as j on
    cj.job_id = j.job_id
  left join {{ source('ats_public', 'job_boards_postings') }} posting on
    cj.vendor_posting_id = posting.vendor_posting_id
    and not posting._fivetran_deleted
  left join {{ source('ats_public', 'job_boards_portals') }} portal on
    posting.job_boards_portal_id = portal.id
    and not portal._fivetran_deleted
  left join {{ source('ats_public', 'career_page_jobs') }} cpj on
    cj.career_page_job_id = cpj.id
  left join {{ source('ats_public', 'job_boards_job_adverts') }} jbja on
