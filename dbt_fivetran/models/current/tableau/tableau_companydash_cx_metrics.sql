...
with
    dates as (
        ...
        from ({{ dbt_utils.generate_series(upper_bound=761) }})
    ),
    ces as (
        select distinct
        ...
        from {{ ref("ces_responses") }} r
        left join {{ source("eh_product", "module_ownership") }} mo on lower(mo.event_module) = lower(r.module)
        group by 1, 2
    ),
    product_mapping as (
        select
        ...
        from {{ source("eh_product", "product_ownership") }} as po
        left join {{ source("eh_product", "workstream_ownership") }} as wo on po.workstream = wo.workstream
        order by po.hr_page
    ),
    ticket as (
        select distinct
        ...
        left join {{ source("zendesk", "ticket_tag") }} tt on t.id = tt.ticket_id
        left join product_mapping po on tt."tag" = po.feature_tag or tt."tag" = po.legacy_tag
        left join {{ source("zendesk", "group") }} g on t.group_id = g.id
        group by 1, 2
    )
select
    d.date,
