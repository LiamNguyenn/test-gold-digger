with
    survey_data as (
        ...
        from {{ source("survey_services_public", "custom_surveys") }} s
        left join
            {{ source("survey_services_public", "custom_survey_submissions") }} ss
            on ss.custom_survey_id = s.id
            and ss._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "answers") }} a on ss.answer_id = a.id and a._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "answer_details") }} ad
            on a.id = ad.answer_id
            and ad._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "single_choice_answer_contents") }} cac
            on ad.content_id = cac.id
            and cac._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "choice_contents") }} cc
            on cc.id = cac.choice_content_id
            and cc._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "text_answer_contents") }} tc
            on tc.id = ad.content_id
            and tc._fivetran_deleted = false
        left join
            {{ source("survey_services_public", "questions") }} q
...
select
    ...
    s.score,
    s.free_text_value
from {{ ref("tableau_companydash_eh_members_managers") }} em
inner join
