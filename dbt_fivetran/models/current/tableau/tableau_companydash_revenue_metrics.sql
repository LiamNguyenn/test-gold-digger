...
fx as (
    select distinct
        ...
        as fx_rate
    from
        {{ source('exchange_rates', 'rate') }} as r
    left join (
        select distinct
...
billed_revenue as (
    ...
    from
        (
            ...
            from
                {{ source('zuora', 'account') }} as a
            inner join {{ source('zuora', 'invoice') }}
                on
                    a.id = invoice.account_id
            inner join {{ source('zuora', 'invoice_item') }}
                on
                    invoice.id = invoice_item.invoice_id
            inner join {{ source('zuora', 'subscription') }}
                on
                    invoice_item.subscription_id = subscription.id
            inner join {{ source('zuora', 'rate_plan_charge') }} as rpc
                on
                    rpc.id = invoice_item.rate_plan_charge_id
            inner join {{ source('zuora', 'product_rate_plan') }} as prp
                on
                    rpc.product_rate_plan_id = prp.id
            inner join {{ source('zuora', 'product') }} as p
                on
                    prp.product_id = p.id
...
employee_creations as (
    ...
    from
        {{ ref('one_platform_employees') }} as e
    left join {{ ref('employment_hero_organisations') }} as o1
        on
            e.eh_organisation_id = o1.id
            and e.eh_member_id is not null
    left join {{ ref('one_platform_organisations') }} as o2
        on
            e.kp_business_id = o2.kp_business_id
            and e.eh_member_id is null
            and e.kp_employee_id is not null
    left join {{ source('zuora', 'account') }} as za
        on
...
accumulation as (
    ...
    as country
    cross join (
        select dateadd('day', 1 - n, current_date) as date
        from
            (
...
accumulation_org as (
    ...
    from
        {{ ref('one_platform_organisations') }}
    cross join (
        select dateadd('month', 1 - n, date_trunc('month', getdate())) as month
        from
            (
                select row_number() over () as n
                from
